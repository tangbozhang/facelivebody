#ifndef _HOLIDAY_BATCH_NORMALISE_CPU_H__
#define _HOLIDAY_BATCH_NORMALISE_CPU_H__
...
template <class	T>
class HolidayBatchNormalizeCPU : public HolidayBaseLayer<T>
{
public:
	HolidayBatchNormalizeCPU();
	~HolidayBatchNormalizeCPU();
...
template <class	T>
int HolidayBatchNormalizeCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
...
template <class	T>
int HolidayBatchNormalizeCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *pNetResource)
{
	m_meanvalue.clear();
	int length_mean = inputparam.batchnormlise_param().mean_param().data_size();
	for (int i = 0; i < length_mean;i++)
	{
		auto tmp_mean_value = inputparam.batchnormlise_param().mean_param().data(i);
	...
	int length_covariance = inputparam.batchnormlise_param().covariance_param().data_size();
	for (int i = 0; i < length_covariance; i++)
	{
		auto tmp_varines_value = inputparam.batchnormlise_param().covariance_param().data(i);
	...
	//m_varines_value = inputparam.varines_value;
...
template <class	T>
HolidayBatchNormalizeCPU<T>::HolidayBatchNormalizeCPU()
{
