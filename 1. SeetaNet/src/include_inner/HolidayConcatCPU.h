#ifndef _HOLIDAY_CONCAT_CPU_H__
#define _HOLIDAY_CONCAT_CPU_H__
...
template <class	T>
class HolidayConcatCPU : public HolidayBaseLayer<T>
{
public:
	HolidayConcatCPU();
	~HolidayConcatCPU();
...
template <class	T>
HolidayConcatCPU<T>::HolidayConcatCPU()
{
...
template <class	T>
int HolidayConcatCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *pNetResource)
{
	...
	//concat_axis_ = inputparam.ConcatParam.axises[0];
	concat_axis_ = inputparam.concat_param().axis();
...
template <class	T>
int HolidayConcatCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
