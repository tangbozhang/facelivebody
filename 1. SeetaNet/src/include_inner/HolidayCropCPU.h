#ifndef __HOLIDAY_CROP_CPU_H_
#define __HOLIDAY_CROP_CPU_H_


#include "HolidayBaseLayer.h"

template <class	T>
class HolidayCropCPU : public HolidayBaseLayer<T>
{
public:
	HolidayCropCPU();
	~HolidayCropCPU();
...
public:
	...
	void crop_copy(const std::vector<HolidayFeatureMap<T>*>& bottom,
		const std::vector<HolidayFeatureMap<T>*>& top,
...
template <class	T>
HolidayCropCPU<T>::HolidayCropCPU()
{
...
template<class T>
int HolidayCropCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
...
template<class T>
int HolidayCropCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *p_holiday_net_resource)
{
	int bottom_index_0 = inputparam.bottom_index(0);
	int bottom_index_1 = inputparam.bottom_index(1);
	HolidayDataSize bottom_size_0 = p_holiday_net_resource->feature_vector_size[bottom_index_0];
	HolidayDataSize bottom_size_1 = p_holiday_net_resource->feature_vector_size[bottom_index_1];

	int start_axis = inputparam.crop_parameter().axis();
	if (start_axis < 0)
	{
	...
	for (int i = 0; i < bottom_size_0.data_dim.size(); ++i) {
		...
		if (i >= start_axis_) {
			...
			if (inputparam.crop_parameter().offset_size() == 1) {
				// If only one offset is given, all crops have the same offset.
				crop_offset = inputparam.crop_parameter().offset(0);
			}
			else if (inputparam.crop_parameter().offset_size() > 1) {
				...
				crop_offset = inputparam.crop_parameter().offset(i - start_axis_);
			}
...
void HolidayCropCPU<Dtype>::crop_copy(const std::vector<HolidayFeatureMap<Dtype>*>& bottom,
	const std::vector<HolidayFeatureMap<Dtype>*>& top,
	const std::vector<int>& offsets,
	std::vector<int> indices,
	int cur_dim,
	...
	if (cur_dim + 1 < top[0]->data_shape.size()) {
		...
		for (int i = 0; i < top[0]->data_shape[cur_dim]; ++i) {
			...
			// do the copy
			if (is_forward) {
				caffe_copy(top[0]->data_shape[cur_dim],
