#ifndef __HOLIDAY_ELTWISE_CPU_H__
#define __HOLIDAY_ELTWISE_CPU_H__
...
template <class	T>
class HolidayEltwiseCPU :public HolidayBaseLayer<T>
{
public:
	enum EltwiseType
	{
...
template <class	T>
HolidayEltwiseCPU<T>::HolidayEltwiseCPU()
{
...
template <class	T>
int HolidayEltwiseCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *pNetResource)
{
	...
	m_elt_type = inputparam.eltwise_param().operation();
	//m_elt_type = inputparam.eltwiseParameter.eltwise_operation;
	int coeff_length = inputparam.eltwise_param().coeff_size();
	m_eltwise_coeff.clear();
	for (int i = 0; i < coeff_length;i++)
	{
		m_eltwise_coeff.push_back(inputparam.eltwise_param().coeff(i));
	}
...
template <class	T>
int HolidayEltwiseCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
	...
	else if (EltwiseParameter_EltwiseOp_SUM == m_elt_type)
	{
	...
	else if (EltwiseParameter_EltwiseOp_MAX == m_elt_type)
	{
