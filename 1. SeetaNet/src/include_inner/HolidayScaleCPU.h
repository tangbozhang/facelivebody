#ifndef __HOLIDAY_SCALE_CPU_H__
#define __HOLIDAY_SCALE_CPU_H__
...
template <class	T>
class HolidayScaleCPU :public HolidayBaseLayer<T>
{
public:
	HolidayScaleCPU();
	~HolidayScaleCPU();
...
template <class	T>
HolidayScaleCPU<T>::HolidayScaleCPU()
{
...
template <class	T>
int HolidayScaleCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *pNetResource)
{
	
	m_scale_value.clear();
	int length_mean = inputparam.scale_param().scale_param().data_size();
	for (int i = 0; i < length_mean; i++)
	{
		auto tmp_scale_value = inputparam.scale_param().scale_param().data(i);
		if (tmp_scale_value < FLT_EPSILON && -tmp_scale_value < FLT_EPSILON) tmp_scale_value = 0;
		m_scale_value.push_back(tmp_scale_value);
	}
	m_bias_value.clear();
	int length_covariance = inputparam.scale_param().bias_param().data_size();
	for (int i = 0; i < length_covariance; i++)
	{
		auto tmp_bias_value = inputparam.scale_param().bias_param().data(i);
...
template <class	T>
int HolidayScaleCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
