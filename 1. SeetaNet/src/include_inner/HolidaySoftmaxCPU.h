#ifndef _HOLIDAY_SOFTMAX_CPU_H__
#define _HOLIDAY_SOFTMAX_CPU_H__
...
template <class	T>
class HolidaySoftMaxCPU :public HolidayBaseLayer<T>
{
public:
	HolidaySoftMaxCPU(){};
	~HolidaySoftMaxCPU(){};
...
public:
	...
	int SoftMaxOperation_s(int number, HolidayBlobCpu<T>& inputData, HolidayBlobCpu<T>& outputData);

	int SoftMaxOperation_Axis2_s(int number, HolidayBlobCpu<T>& inputData, HolidayBlobCpu<T>& outputData);
private:
	...
	int32_t engine;
public:
	typedef int (HolidaySoftMaxCPU<T>::*pDetailProcessFun)(int number, HolidayBlobCpu<T>& inputData, HolidayBlobCpu<T>& outputData);
	pDetailProcessFun m_pdetailProcessFun;

	int count_out_permute_dim(int start_axis, int end_axis, std::vector<int> input_shape);

	HolidayBlobCpu<T> max_value_blob;
	HolidayBlobCpu<T> sum_value_blob;

	int outer_num_;
	int inner_num_;
	int softmax_axis_;
	/// sum_multiplier is used to carry out sum using BLAS
	HolidayBlobCpu<T> sum_multiplier_;
	/// scale is an intermediate Blob to hold temporary results.
	HolidayBlobCpu<T> scale_;
};

template <class	T>
int HolidaySoftMaxCPU<T>::count_out_permute_dim(int start_axis, int end_axis, std::vector<int> input_shape)
{
	int out_count = 1;
...
template <class	T>
int HolidaySoftMaxCPU<T>::Init(Holiday_LayerParameter& inputparam, HolidayNetResource<T> *pNetResource)
{
	...
	axis = inputparam.softmax_param().axis();	
	...
	if (1 == axis)
	{
		m_pdetailProcessFun = &HolidaySoftMaxCPU<T>::SoftMaxOperation_s;
	}
	else if (2 == axis)
	{
		m_pdetailProcessFun = &HolidaySoftMaxCPU<T>::SoftMaxOperation_Axis2_s;
	}
	return 0;
}

template <class	T>
int HolidaySoftMaxCPU<T>::Process(std::vector<HolidayFeatureMap<T>*> input_data_map, std::vector<HolidayFeatureMap<T>*>& output_data_map)
{
	...
	caffe_set(sum_multiplier_.count(), T(1), multiplier_data);
	...
	caffe_copy(bottom_counts, bottom_data, top_data);
	...
	for (int i = 0; i < outer_num_; ++i) {
		...
		// subtraction
		caffe_cpu_gemm<T>(CblasNoTrans, CblasNoTrans, channels, inner_num_,
			1, -1., sum_multiplier_.dataMemoryPtr(), scale_data, 1., top_data);
		// exponentiation
		caffe_exp<T>(dim, top_data, top_data);
		// sum after exp
		caffe_cpu_gemv<T>(CblasTrans, channels, inner_num_, 1.,
			top_data, sum_multiplier_.dataMemoryPtr(), 0., scale_data);
		// division
		for (int j = 0; j < channels; j++) {
...
template<typename T>
int HolidaySoftMaxCPU<T>::SoftMaxOperation_s(int number, HolidayBlobCpu<T>& inputData, HolidayBlobCpu<T>& outputData)
{
	HolidayBlobCpu<T> tmp_cube = inputData;
...
template <class	T>
int HolidaySoftMaxCPU<T>::SoftMaxOperation_Axis2_s(int number, HolidayBlobCpu<T>& inputData, HolidayBlobCpu<T>& outputData)
{
	std::vector<int> position_index;
	position_index.resize(4);
	std::vector<int> position_index1;
