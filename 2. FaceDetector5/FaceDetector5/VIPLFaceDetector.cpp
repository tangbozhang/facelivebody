...
#else
#error Please update SeetaNet to 0.7.5 to use SeetaModelResetInput
#endif
...
#define LIBRARY_VERSION GENERATE_VER( \
	(VIPL_FACE_DETECTOR_MAJOR_VERSION) \
	(VIPL_FACE_DETECTOR_MINOR_VERSION) \
	(VIPL_FACE_DETECTOR_SUBMINOR_VERSION))
...
public:
	...
	// removed, not used
    // void LoadModel(const char* model_path12, const char* model_path24, const char* model_path48, VIPLFaceDetector::Device device);
	void LoadModelBuffer(const char* model_buffer,
	                     int64_t buffer_lenght12,
	                     int64_t buffer_lenght24,
	                     int64_t buffer_lenght48, VIPLFaceDetector::Device device);

    inline bool IsLegalImage(const VIPLImageData & image)
    {
    ...
    bool Legal(int x, int y, const VIPLImageData & img)
    {
    ...
    std::vector<VIPLFaceInfo> TransWindow(const VIPLImageData & img, const VIPLImageData & img_pad, std::vector<Rect> &winList);

    std::vector<Rect> SlidingWindow(const VIPLImageData & img, const VIPLImageData & img_pad, SeetaCNN_Net* &net, float thres, int local_min_face = -1, int local_max_face = -1);

    std::vector<Rect> RunNet(const VIPLImageData & img, SeetaCNN_Net* &net, float thres, int dim, std::vector<Rect> &winList);

    void SetInput(const VIPLImageData & img, int dim, std::vector<Rect> &winList, uint8_t *tmp_data);

public:
    int min_face_;
    float scale_;
    float class_threshold_[3];
    SeetaCNN_Model *model_[3];
    SeetaCNN_Net *net_[3];
    int stride_ = 4;
...
std::vector<Rect> Impl::SlidingWindow(const VIPLImageData & img, const VIPLImageData & img_pad, SeetaCNN_Net* &net, float thres, int local_min_face, int local_max_face)
{
    ...
    VIPLImageData img_resized(int(img.width / cur_scale), int(img.height / cur_scale), img.channels);
    ...
    while (std::min(img_resized.width, img_resized.height) >= net_size)
    {
        ...
        SeetaCNN_InputOutputData reg_res, cls_res;
...
void Impl::SetInput(const VIPLImageData & img, int dim, std::vector<Rect> &winList, uint8_t *tmp_data)
{
    VIPLImageData tmp(dim, dim, img.channels);
    //tmp.data = new uint8_t[tmp.channels * tmp.height * tmp.width];
    for (size_t i = 0; i < winList.size(); i++)
    {
        ...
        // memcpy(tmp_data + i * tmp.channels * tmp.height * tmp.width, tmp.data, sizeof(unsigned char) * tmp.channels * tmp.height * tmp.width);
    }
    //delete[] tmp.data;
}

std::vector<Rect> Impl::RunNet(const VIPLImageData & img, SeetaCNN_Net* &net, float thres, int dim, std::vector<Rect> &winList)
{
    if (winList.size() == 0)
        return winList;
    std::vector<Rect> ret;
...
std::vector<VIPLFaceInfo> Impl::TransWindow(const VIPLImageData & img, const VIPLImageData & img_pad, std::vector<Rect> &winList)
{
    int row = (img_pad.height - img.height) / 2;
    int col = (img_pad.width - img.width) / 2;

    std::vector<VIPLFaceInfo> ret;
    for (size_t i = 0; i < winList.size(); i++)
    {
        winList[i].x -= col;
        winList[i].y -= row;
...
void Impl::LoadModelBuffer(const char* model_buffer, int64_t buffer_lenght12, int64_t buffer_lenght24, int64_t buffer_lenght48, VIPLFaceDetector::Device device)
{
	using self = VIPLFaceDetector;
	...
	if (type == HOLIDAY_CNN_GPU_DEVICE) SeetaCreateNetGPU(model_[0], 1, gpu_id, &net_[0]);
	else SeetaCreateNet(model_[0], 1, type, &net_[0]);

    SeetaReadModelFromBuffer(model_buffer24, size_t(buffer_lenght24), &model_[1]);
	if (type == HOLIDAY_CNN_GPU_DEVICE) SeetaCreateNetGPU(model_[1], max_batch_size[0], gpu_id, &net_[1]);
	else SeetaCreateNet(model_[1], max_batch_size[0], type, &net_[1]);

    SeetaReadModelFromBuffer(model_buffer48, size_t(buffer_lenght48), &model_[2]);
	if (type == HOLIDAY_CNN_GPU_DEVICE) SeetaCreateNetGPU(model_[2], max_batch_size[1], gpu_id, &net_[2]);
	else SeetaCreateNet(model_[2], max_batch_size[1], type, &net_[2]);
}
                       ...
                       int crop_x, int crop_y, int crop_w, int crop_h)
{
    ...
    {
        auto bins = orz::split_bins(0, dst_height * dst_width, int(gun->size()));
        for (auto &bin : bins)
        {
            gun->fire([&, bin](int)
            {
...
void VIPLFaceDetector::SetNumThreads(int num)
{
	SeetaSetNumThreads(num);
}

VIPLFaceDetector::VIPLFaceDetector(const char* model_path)
	: VIPLFaceDetector(model_path, AUTO)
{
}

VIPLFaceDetector::VIPLFaceDetector(const char* model_path, Device device)
	: VIPLFaceDetector(model_path, CoreSize(-1, -1), device)
{
}

VIPLFaceDetector::VIPLFaceDetector(const char* model_path, const CoreSize& core_size)
	: VIPLFaceDetector(model_path, CoreSize(-1, -1), AUTO)
{
}
...
{
	auto model_marks = orz::Split(model_path, '.');
	//static const std::regex rgx(R"(([0-9]*[kK]?)x([0-9]*[kK]?))");
...
VIPLFaceDetector::VIPLFaceDetector(const char* model_path, const CoreSize &core_size, Device deviece)
    : impl_(new Impl())
{
...
// Code
    ...
    orz::imemorystream in(bin.data(), bin.size());

	Impl *p = (Impl *)impl_;

	static const int width_limit = 640;
	static const int height_limit = 480;
	...
	if (mark == 0x19910929)
	{
		...
		p->sta.version = orz::jug_get<std::string>(model["version"], "0.0.0");
		auto vi = version(p->sta.version);
		...
		p->sta.date = orz::jug_get<std::string>(model["date"]);
		p->sta.name = orz::jug_get<std::string>(model["name"]);
		sta_data = orz::jug_get<std::string>(model["data"]);

		if (sta_data.empty()) throw orz::Exception("Loading an empty model");

		auto params = model["params"];
		p->stride_ = orz::jug_get<int>(params["stride"], p->stride_);
		p->end2end_ = orz::jug_get<int>(params["end2end"], p->end2end_) != 0;
		p->thresh1_ = orz::jug_get<float>(params["thresh1"], p->thresh1_);
		p->thresh2_ = orz::jug_get<float>(params["thresh2"], p->thresh2_);
		p->thresh3_ = orz::jug_get<float>(params["thresh3"], p->thresh3_);

		auto input = model["input"];
		// p->width_limit_ = orz::jug_get<int>(input["width_limit"], p->width_limit_);
		// p->height_limit_ = orz::jug_get<int>(input["height_limit"], p->height_limit_);
		
		// this buffer for read only use
		buffer = const_cast<char*>(sta_data.data());
	}
	else
	{
		in.seekg(0, std::ios::beg);
		in.seekg(0, std::ios::end);
		auto sta_length = in.tellg();
		sta_buffer.reset(new char[size_t(sta_length)], std::default_delete<char[]>());
		in.seekg(0, std::ios::beg);
		in.read(sta_buffer.get(), sta_length);
	...
	int offset = 12;
	p->LoadModelBuffer(buffer + offset, length1, length2, length3, deviece);

	SetMinFaceSize(40);
	SetScoreThresh(p->thresh1_, p->thresh2_, p->thresh3_);
	SetImagePyramidScaleFactor(1.414f);

	orz::Log(orz::STATUS) << LOG_HEAD << "Core size: " << p->width_limit_ << "x" << p->height_limit_;
}


VIPLFaceDetector::~VIPLFaceDetector()
{
    Impl *p = (Impl *)impl_;
    if (!p) return;
...
void VIPLFaceDetector::SetVideoStable(bool stable)
{
	Impl *p = (Impl *)impl_;
...
bool VIPLFaceDetector::GetVideoStable() const
{
	Impl *p = (Impl *)impl_;
	return p->stable_;
}

VIPLFaceDetector::CoreSize VIPLFaceDetector::GetCoreSize() const
{
	Impl *p = (Impl *)impl_;
...
std::vector<VIPLFaceInfo> VIPLFaceDetector::Detect(const VIPLImageData & image)
{
#ifdef SEETA_CHECK_AUTO_FUNCID
	SEETA_CHECK_AUTO_FUNCID("FaceDetector");
#endif
    Impl *p = (Impl *)impl_;
	if (!p->IsLegalImage(image))
        return std::vector<VIPLFaceInfo>();

    orz::ctx::lite::bind<orz::Shotgun> _bind_gun(p->m_gun.get());
    ...
    VIPLImageData img_pad(img.width() + 2 * pad_w, img.height() + 2 * pad_h, img.channels());
    ...
    delete[] img_pad.data;
    return p->TransWindow(image, image, winList);//暂时不用
}

void VIPLFaceDetector::SetMinFaceSize(int32_t size)
{
    Impl *p = (Impl *)impl_;
...
void VIPLFaceDetector::SetImagePyramidScaleFactor(float factor)
{
    Impl *p = (Impl *)impl_;
    p->scale_ = factor > 1.414f ? factor : 1.414f;
}

void VIPLFaceDetector::SetScoreThresh(float thresh1, float thresh2, float thresh3)
{
    Impl *p = (Impl *)impl_;
...
int32_t VIPLFaceDetector::GetMinFaceSize() const
{
    Impl *p = (Impl *)impl_;
    return p->min_face_;
}

float VIPLFaceDetector::GetImagePyramidScaleFactor() const
{
    Impl *p = (Impl *)impl_;
    return p->scale_;
}

void VIPLFaceDetector::GetScoreThresh(float* thresh1, float* thresh2, float* thresh3) const
{
    Impl *p = (Impl *)impl_;
