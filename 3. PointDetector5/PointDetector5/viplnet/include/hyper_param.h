...
class HyperParam {
 ...
 private:
  std::string read_str(orz::FILE* file) {
    int len;
    CHECK_EQ(fread(&len, sizeof(int), 1, file), 1);
    if (len <= 0) return "";
    char* c_str = new char[len + 1];
    CHECK_EQ(fread(c_str, sizeof(char), size_t(len), file), size_t(len));
    c_str[len] = '\0';
    std::string str(c_str);
    delete []c_str;
    return str;
  }
  int read_int(orz::FILE* file) {
    int i;
    CHECK_EQ(fread(&i, sizeof(int), 1, file), 1);
    return i;
  }
  float read_float(orz::FILE* file) {
    float f;
