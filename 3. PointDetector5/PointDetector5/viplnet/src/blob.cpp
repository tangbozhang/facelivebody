...
Blob::Blob(orz::FILE* file) {
  ...
  data_.reset(new float[count_], std::default_delete<float[]>());
  CHECK_EQ(fread(data_.get(), sizeof(float), size_t(count_), file), size_t(count_));
}
...
void Blob::CopyTo(unsigned char* const data) {
  ...
  for (int i = 0; i < count_; ++ i)
    data[i] = static_cast<unsigned char>(std::min(255.0f, std::max(0.0f, data_head[i])));
}
...
void Blob::ToBinaryFile(const std::string file_name) {
  FILE* file = nullptr;
  auto open_failed = fopen_s(&file, file_name.c_str(), "wb");
  (void)(open_failed);
  if (file == nullptr) {
