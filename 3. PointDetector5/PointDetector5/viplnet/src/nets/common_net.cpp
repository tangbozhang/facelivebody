...
std::shared_ptr<Net> CommonNet::Load(orz::FILE* file) {
	// make sure link layers
	{
		BiasAdderNet bias_adder;
		BnNet bn;
		ConvNet conv;
		EltwiseNet eltwise;
		EltwiseOPNet eltwise_op;
		InnerProductNet inner_product;
		MaxPoolingNet max_pooling;
		PadNet pad;
		ReLUNet relu;
		ScaleNet scale;
		ShapeIndexPatchNet shape_index_patch;
		SpatialTransformNet spatial_transform;
		TransformationMakerNet tfrom_maker;
	}
  ...
  // net type
  CHECK_EQ(fread(net_type, sizeof(char), size_t(len), file), size_t(len));
  net_type[len] = '\0';
  ...
  for (size_t i = 0; i < net->params().size(); ++ i) {
    ...
    net->params(int(i))->SetData(param);
  }

  size_t num_subnet = net->nets().size();
  size_t num_in = net->input_blobs().size();
  size_t num_out = net->output_blobs().size();
  ...
  if (num_subnet == 0) {
    ...
    // connection of each subnet
      for (size_t i = 0; i < num_subnet; ++i) {
      size_t input_num = nets[i]->input_blobs().size();
      for (size_t j = 0; j < input_num; ++j) {
        ...
        if (net_idx == -1) { // connected to father net
          input_plugs[blob_idx].push_back(nets[i]->input_blobs(int(j)));
        }
        else {
          nets[net_idx]->output_plugs(blob_idx).push_back(
            nets[i]->input_blobs(int(j))); 
        }
      }
    }
    // get output blobs
