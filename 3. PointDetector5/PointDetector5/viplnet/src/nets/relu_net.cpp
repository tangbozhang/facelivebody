...
void ReLUNet::Execute() {
  CheckInput();
  const Blob &feat_blob = input_blobs_[0];
  float *const buff = new float[feat_blob.count()];
  for (int i = 0; i < feat_blob.count(); i++) {
    buff[i] = feat_blob[i] > 0 ? feat_blob[i] : 0;
  }
  output_blobs_[0].CopyData(feat_blob.num(), feat_blob.channels(), feat_blob.height(), feat_blob.width(), buff);
  delete[] buff;
  CheckOutput();
}

REGISTER_NET_CLASS(ReLU);
