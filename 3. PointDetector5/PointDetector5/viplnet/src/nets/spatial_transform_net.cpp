        ...
        for (int c = 0; c < channels; ++ c) {
          if (!is_mat_data_) {
            output_data[output->offset(n, c, x, y)]
              = (float)Sampling(input_data + input->offset(n, c), src_h, src_w,
              src_x, src_y, 1.0 / scale);
          }
          else {
            output_data[output->offset(n, c, x, y)]
                = (float)Sampling(reinterpret_cast<unsigned char*>(
...
double SpatialTransformNet::Sampling(const float* const feat_map, int H, int W, 
  ...
  if (type_ == "linear") {
    ...
    int ux = int(floor(x)), uy = int(floor(y));
    double ans = 0;
    ...
    if (x >= 0 && x < H && y >= 0 && y < W) {
      ...
      for (int ux = (int)ceil(x - kernel_width / 2); 
          (double)ux <= floor(x + kernel_width / 2); ++ ux) {
        indices_x.push_back(std::max(std::min(H - 1, ux), 0));
        weights_x.push_back(Cubic((x - ux) * scale));
      }
      // get indices and weight along y axis
       for (int uy = (int)ceil(y - kernel_width / 2); 
          (double)uy <= floor(y + kernel_width / 2); ++ uy) {
