...
static std::vector<VIPLImageData> jni_convert_image_datas(JNIEnv* env, jobjectArray image_datas)
{
	jsize size = env->GetArrayLength(image_datas);
	std::vector<VIPLImageData> native_images(size);
	for(jsize i = 0; i < size; ++i)
	{
		jobject image_data = env->GetObjectArrayElement(image_datas, i);
		native_images[i] = jni_convert_image_data(env, image_data);
	}
...
static void jni_convert_points(JNIEnv *env, jobjectArray landmarks, VIPLPoint native_points[], int length)
{
	jsize landmarks_size = env->GetArrayLength(landmarks);
	if (landmarks_size < 5) jni_throw(env, "Landmarks have at least 5 points");
	for (jsize i = 0; i < length; ++i)
	{
		jobject point = env->GetObjectArrayElement(landmarks, i);
		native_points[i] = jni_convert_point(env, point);
	}
}

static void jni_convert_points(JNIEnv *env, jobjectArray landmarks, std::vector<VIPLPoint> &native_points, int length)
{
	jsize landmarks_size = env->GetArrayLength(landmarks);
	if (landmarks_size < 5) jni_throw(env, "Landmarks have at least 5 points");
	for (jsize i = 0; i < length; ++i)
