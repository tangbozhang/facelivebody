	...
	namespace v500
	{
		...
		public:
            ...
            Implement(const SeetaModelSetting &setting, int extraction_core_number, int comparation_core_number)
			{
                ...
                auto shared_model = seeta::FaceRecognizer::LoadModel(setting);
                orz::need free_shared_model(seeta::FaceRecognizer::FreeModel, shared_model);

                m_cores.resize(extraction_core_number);
                for (auto &core : m_cores)
                {
                    core = std::make_shared<seeta::FaceRecognizer>(shared_model);
                }
...
void seeta::FaceDatabase::SetSingleCalculationThreads(int num)
{
    FaceRecognizer::SetSingleCalculationThreads(num);
}

int seeta::FaceDatabase::GetCropFaceWidth()
{
    return 256;
}

int seeta::FaceDatabase::GetCropFaceHeight()
{
    return 256;
}

int seeta::FaceDatabase::GetCropFaceChannels()
{
    return 3;
}

bool seeta::FaceDatabase::CropFace(const SeetaImageData& image, const SeetaPointF* points, SeetaImageData& face)
{
