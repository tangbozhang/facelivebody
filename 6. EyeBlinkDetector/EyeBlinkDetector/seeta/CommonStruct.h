...
namespace seeta
{
	...
	public:
...
#ifdef VIPL_SUPPORT
		template <size_t _N>
		Landmarks(const VIPLPoint(&points)[_N]) : Landmarks(points, _N) {}
		Landmarks(const VIPLPoint *points, size_t _N)
			: points(_N)
		{
			for (int i = 0; i < _N; ++i) {
		...
		void copy_to(VIPLPoint *points, int N = -1)
		{
			if (N < 0) N = static_cast<int>(this->points.size());
			for (int i = 0; i < N; ++i) {
				points[i].x = this->points[i].x;
				points[i].y = this->points[i].y;
			}
		}
#endif
		std::vector<PointF> points;
...
#ifdef INCLUDED_SEETA_CSTRUCT
		Image(const SeetaImageData &vimg) : Image(vimg.data, vimg.width, vimg.height, vimg.channels) {}
		operator SeetaImageData() const { SeetaImageData vimg = { width(), height(), channels(), const_cast<Datum *>(data()) }; return vimg; }
#endif
#ifdef INCLUDED_SEETA_STRUCT
		Image(const ImageData &vimg) : Image(vimg.data, vimg.width, vimg.height, vimg.channels) {}
		operator ImageData() const { ImageData vimg(const_cast<Datum *>(data()), width(), height(), channels()); return std::move(vimg); }
#endif
#ifdef VIPL_SUPPORT
		Image(const VIPLImageData &vimg) : Image(vimg.data, vimg.width, vimg.height, vimg.channels) {}
		operator VIPLImageData() const { VIPLImageData vimg(width(), height(), channels()); vimg.data = const_cast<Datum *>(data()); return vimg; }
#endif
	...
	inline void _out_str(std::ostream &out, const T &t, Args... args) { _out_str(out << t, args...); }
