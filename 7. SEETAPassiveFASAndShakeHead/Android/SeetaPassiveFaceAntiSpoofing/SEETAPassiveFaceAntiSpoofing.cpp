...
SEETAPassiveFaceAntiSpoofing::SEETAPassiveFaceAntiSpoofing(const std::string ModelPath)
{
	const std::string modelFileName = ModelPath + "SeetaAntiSpoofing1.0.4.RN18.M3D8.sta"; 
    output_blob_name2timesFace = "prob";
	auto bin = orz::read_file(modelFileName);
	orz::imemorystream in(bin.data(), bin.size());

	auto model = orz::sta_read(in);
	if (!model.valid()) orz::Log(orz::ERROR) << "Can not access " << modelFileName << ", file may broken." << orz::crash;

	auto version = orz::jug_get<std::string>(model["version"], "");
	if (version != "1.0.4") orz::Log(orz::ERROR) << "Unsupported model version: " << version << orz::crash;

	auto data = model["data"];
	if (!data.valid(orz::Piece::LIST) || data.size() != 1) orz::Log(orz::ERROR) << "Got an broken model " << modelFileName << orz::crash;
	auto buffer2timesFace = data[0].to_binary();

	SeetaCNN_Model *model2timesFace = nullptr;
	SeetaReadModelFromBuffer(buffer2timesFace.data<char>(), buffer2timesFace.size(), &model2timesFace);
	orz::need releaseModel2timesFace(SeetaReleaseModel, model2timesFace);

    SeetaCreateNet(model2timesFace, 1, HOLIDAY_CNN_CPU_DEVICE, &net2timesFace);

	orz::Log(orz::INFO) << "Model status:" << std::boolalpha << " " << (net2timesFace != nullptr);
