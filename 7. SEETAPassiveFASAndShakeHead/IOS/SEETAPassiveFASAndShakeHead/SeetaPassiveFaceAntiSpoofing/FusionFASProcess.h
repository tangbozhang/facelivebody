...
public:
    ...
    FusionFASProcess(const SEETAPassiveFASAndShakeHead::ModelSetting &model_setting,
        const int shakeHeadAngleThreshold,
        const int nodHeadAngleThreshold,
        const double clarityTheshold,
        const double fuseThreshold,
    ...
    VIPLImageData getSnapshot(VIPLFaceInfo &face);
	void resetSnapshot();
    ...
    void updateSnapshot(const VIPLImageData &image, const VIPLFaceInfo& face, const std::vector<VIPLPoint> &points, float score);
    void updateSnapshot(const VIPLImageData& image, const VIPLFaceInfo& info, const std::vector<VIPLPoint> &points, const  float &yawAngle, const  float &pitchAngle, const  float & rollAngle, const double &clarity);

    void setVerifyingThreshold(float threshold = 0.7) { verifying_threshold = threshold; }
private:
	VIPLImageData snapshot;
	std::shared_ptr<uint8_t> snapshot_data;
	float snapshot_score = 0;
	size_t snapshot_length = 0;
    VIPLFaceInfo snapshot_face;
    std::vector<VIPLPoint> snapshot_points;

    seeta::Image verifying_image;
    seeta::Rect verifying_face;
    std::vector<seeta::PointF> verifying_points;

    float verifying_threshold = 0.7;

    std::shared_ptr<VIPLFaceRecognizer> face_recognizer;

	std::shared_ptr<CommonPoseEstimation> poseEstimation;
	std::shared_ptr<CommonEyeBlinkDetector> closeEyeDetector;
