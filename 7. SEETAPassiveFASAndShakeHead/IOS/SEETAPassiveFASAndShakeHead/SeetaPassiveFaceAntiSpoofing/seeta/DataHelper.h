...
namespace seeta
{
	...
	public:
		...
		{
			reshape(other.shape());
			for (int i = 0; i < count(); ++i) data()[i] = static_cast<T>(other.data()[i]);
			return *this;
		}
		...
		{
			if (axe < 0 || axe >= m_shape.size()) return 1;
			return m_shape[axe];
		}
		...
		{
			for (int i = 0; i < this->count(); ++i) func(this->data()[i]);
			return *this;
		}
		...
		Blob operator-()
		{
			...
			for (int i = 0; i < this->count(); ++i) result.data()[i] = -this->data()[i];
			return result;
		}
		...
		friend Blob operator*(const Blob& blob, const T &value)
		{
			...
			for (int i = 0; i < blob.count(); ++i) result.data()[i] = blob.data()[i] * value;
			return result;
		}
		...
		{
			Blob<T> result(blob.shape());
			for (int i = 0; i < blob.count(); ++i) result.data()[i] = value * blob.data()[i];
			return result;
		}
		...
		const Blob &operator+=(const Blob<T1>& blob)
		{
			for (int i = 0; i < 4; ++i)
			{
				if (shape(i) != blob.shape(i)) throw std::logic_error("Cant not eltwise do blob shape mismatch");
			}
			for (int i = 0; i < count(); ++i) data()[i] += blob.data()[i];
			return *this;
		}
		...
		{
			if (axis < 0 || axis > 3) throw std::logic_error("Can not swap blob at axis " + std::to_string(axis));
			if (dims.empty()) throw std::logic_error("Can not shuffle to empty dim");
			for (auto dim : dims)
			{
	...
	{
		// check axis
		if (axis < 0 || axis > 3) throw std::logic_error("Can not concat blobs at axis " + std::to_string(axis));
		if (blobs.empty()) return Blob<T>();
