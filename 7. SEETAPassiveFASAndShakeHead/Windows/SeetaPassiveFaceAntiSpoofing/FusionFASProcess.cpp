...
void FusionFASProcess::updateSnapshot(const VIPLImageData& image, const VIPLFaceInfo& face, float score)
{
...
static float GetSnapshot(const seeta::Image& frame, seeta::Image &dest, const seeta::Rect& rect, seeta::Rect &location, float scalar)
{
...
void FusionFASProcess::updateSnapshot(const VIPLImageData& image, const VIPLFaceInfo& info, const  float &yawAngle, const  float &pitchAngle, const  float & rollAngle,const double &clarity)
{
	...
	seeta::Image face;
    seeta::Rect location;
	facialQualityValue *=GetSnapshot(seeta::Image(image.data, image.width, image.height, image.channels), face, seeta::Rect(info.x, info.y, info.width, info.height), location, 0.5);
    ...
    updateSnapshot(image, info, facialQualityValue);
}
...
void FusionFASProcess::detecion(VIPLImageData VIPLImage, const VIPLFaceInfo &info, const VIPLPoint* points5, SystemState &state)
{
	...
	int stable = isStable(points5, prepoints,info.width);//人脸是否稳定
	for (int i = 0; i < 5; i++){
	...
	if (frameNum<thisFirstPhaseFrameNum)
	{
		...
		if (isMiddle(yawAngle, pitchAngle, rollAngle))
		{
			if (stable == 1)
			{
                int eyeStatus = closeEyeDetector->Detect(VIPLImage, points5);
				bool isCloseEye = eyeStatus != 0;
				if (!isCloseEye)
				{
				clarity = ClarityEstimator->clarityEstimate(VIPLImage, info);
				passiveFASResult = antiSpoofing->matToTheSeetaNet(VIPLImage, info, points5);//置信度
				firstPhaseVector_clarity.push_back(clarity);
				firstPhaseVector_passiveFASResult.push_back(passiveFASResult);
				frameNum++;
				updateSnapshot(VIPLImage, info, yawAngle, pitchAngle, rollAngle,clarity);
				}
			}
			state = detecting;
		}
