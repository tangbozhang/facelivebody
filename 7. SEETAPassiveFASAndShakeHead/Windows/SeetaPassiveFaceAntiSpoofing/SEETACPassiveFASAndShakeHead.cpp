...
static SystemState SeetaCSystemState2SystemState(SeetaCSystemState state)
{
	...
	case PLEASE_FACE_THE_CAMERA: return pleaseFaceTheCamera;
...
static SeetaCSystemState SystemState2SeetaCSystemState(SystemState state)
{
    ...
    case pleaseFaceTheCamera: return PLEASE_FACE_THE_CAMERA;
...
void SEETACPassiveFASAndShakeHead_getSnapshot_v2(SEETACPassiveFASAndShakeHead* object, VIPLImageData *snapshot, VIPLFaceInfo *face)
{
    if (object == nullptr) return;
    auto impl = reinterpret_cast<SEETAPassiveFASAndShakeHead *>(object);
    ...
    if (face)
    {
        *snapshot = impl->getSnapshot(*face);
    }
    else
    {
        *snapshot = impl->getSnapshot();
