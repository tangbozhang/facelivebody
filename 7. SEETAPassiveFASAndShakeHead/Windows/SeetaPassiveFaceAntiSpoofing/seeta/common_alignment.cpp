	...
	SAMPLING_TYPE type = LINEAR)
{
	...
	else if (type == BICUBIC)
	{
		...
		if (x >= 0 && x < image_height && y >= 0 && y < image_width) {
			...
			int ux_left = std::max<int>(0, static_cast<int>(ceil(x - kernel_width / 2)));
			int ux_right = std::min<int>(image_height - 1, static_cast<int>(floor(x + kernel_width / 2)));
			...
			int uy_left = std::max<int>(0, static_cast<int>(ceil(y - kernel_width / 2)));
			int uy_right = std::min<int>(image_width - 1, static_cast<int>(floor(y + kernel_width / 2)));
		...
		{
			// memset(pixel, 0, sizeof(uint8_t) * image_channels);
			near_sampling(image_data, image_width, image_height, image_channels, x, y, pixel);
		}
	}
	else
	{
	...
	int N = 1)
{
	...
	for (int n = 0; n < N; ++n) {
		double scale = sqrt(theta_data[0] * theta_data[0] + theta_data[3] * theta_data[3]);
		for (int x = 0; x < dst_h; ++x)
			for (int y = 0; y < dst_w; ++y) {
				...
				//	src_y = theta_data[0] * tmp_y + theta_data[1] * tmp_x + theta_data[2];
				//	src_x = theta_data[3] * tmp_y + theta_data[4] * tmp_x + theta_data[5];
